...
## Opt-in vs opt-out
echo -n "LINE: ${LINENO} " && sed < ./zonefile-a > "$INSTALL_ROOT/var/opendnssec/unsigned/ods1" -e 's/SERIAL/1001/g;s/ods./ods1./g' &&
echo -n "LINE: ${LINENO} " && sed < ./zonefile-a > "$INSTALL_ROOT/var/opendnssec/unsigned/ods2" -e 's/SERIAL/1001/g;s/ods./ods2./g' &&
...
echo "Update ods2, add DS and wait for signed zone" &&
echo -n "LINE: ${LINENO} " && sed < ./zonefile-b > "$INSTALL_ROOT/var/opendnssec/unsigned/ods2" -e 's/SERIAL/1002/g;s/ods./ods2./g' &&
echo -n "LINE: ${LINENO} " && ods-signer sign ods2 &&
echo -n "LINE: ${LINENO} " && syslog_waitfor 60 'ods-signerd: .*\[adapter\] write zone ods2 serial 1002*' &&
sleep 10 &&

echo -n "LINE: ${LINENO} " && hash=`ldns-nsec3-hash -a 1 -t 5 aaa.ods2.` &&
echo -n "LINE: ${LINENO} " && grep "${hash}ods2.[[:space:]]*3600[[:space:]]*IN[[:space:]]*NSEC3[[:space:]]*1 1 5 -.* NS DS RRSIG" $INSTALL_ROOT/var/opendnssec/signed/ods2 &&

echo -n "LINE: ${LINENO} " && validns $INSTALL_ROOT/var/opendnssec/signed/ods2 &&

## Empty non-terminal
echo -n "LINE: ${LINENO} " && sed < ./zonefile-c > "$INSTALL_ROOT/var/opendnssec/unsigned/ods3" -e 's/SERIAL/1001/g;s/ods./ods3./g' &&
...
echo -n "LINE: ${LINENO} " && hash=`ldns-nsec3-hash -a 1 -t 5 on.ods3.` &&
echo -n "LINE: ${LINENO} " && grep "${hash}ods3.[[:space:]]*3600[[:space:]]*IN[[:space:]]*NSEC3[[:space:]]*1 1 5 -.*" "$INSTALL_ROOT/var/opendnssec/signed/ods3" &&
echo -n "LINE: ${LINENO} " && hash=`ldns-nsec3-hash -a 1 -t 5 ottawa.on.ods3.` &&
echo -n "LINE: ${LINENO} " && grep "${hash}ods3.[[:space:]]*3600[[:space:]]*IN[[:space:]]*NSEC3[[:space:]]*1 1 5 -.*" "$INSTALL_ROOT/var/opendnssec/signed/ods3" &&
...
echo "Update ods4, add DS and wait for signed zone" &&
echo -n "LINE: ${LINENO} " && sleep 2 &&
echo -n "LINE: ${LINENO} " && sed < ./zonefile-e > "$INSTALL_ROOT/var/opendnssec/unsigned/ods4" -e 's/SERIAL/1002/g;s/ods./ods4./g' &&
echo -n "LINE: ${LINENO} " && ods-signer sign ods4 &&
echo -n "LINE: ${LINENO} " && syslog_waitfor 60 'ods-signerd: .*\[adapter\] write zone ods4 serial 1002*' &&
